...
local Signature			= BinUtil.toSignature("PFS ")
...
function S3D.open(path, reload)
	local data = OpenRaw(path)

	local header = ffi.cast("S3DHeader*", data)
	if header.signature ~= Signature then
		error("file is not a valid s3d/eqg archive")
	end

	local p = header.offset
	local n = ffi.cast(BinUtil.Uint32Ptr, data + p)
	local num_entries = n[0]
	p = p + ffi.sizeof(BinUtil.Uint32)
	...
	n = ffi.cast(BinUtil.Uint32Ptr, name_data)
	n = n[0]
	p = ffi.sizeof(BinUtil.Uint32)

	for i = 1, n do
		local len = ffi.cast(BinUtil.Uint32Ptr, name_data + p)
		len = len[0]
		p = p + ffi.sizeof(BinUtil.Uint32)
...
function S3D:GetCompressedEntry(i)
	local ent = self[i]
	if ent then
		return ent, ffi.sizeof(ent)
	end
end

function S3D:GetCompressedEntryByName(name)
	local i = self.by_name[name]
	if i then return self:GetCompressedEntry(i) end
end

function S3D:SetEntry(i, data, len)
	self.decompressed[i] = data
	self[i].inflatedLen = len
end

function S3D:SetEntryByName(name, data, len)
	local i = self.by_name[name]
	if i then self:SetEntry(i, data, len) end
end

local function name_itr(names, i)
	i = i + 1
	local name = names[i]
...
function S3D:save(override_path)
	if override_path then self.path = override_path end
	...
	local n = ffi.new("uint32_t[1]", #self)
	name_buf:Add(n, ffi.sizeof(BinUtil.Uint32))
	...
	n[0] = #dir_entries
	C.fwrite(n, ffi.sizeof(BinUtil.Uint32), 1, file)
